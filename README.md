...
This project implements a **Lambda Architecture**, a robust approach for processing massive data by combining two flows:

### 1. Speed Layer (Real-Time)
- **Flow**: Kafka → PySpark Streaming → MongoDB.
- **Why "Drip-feed" (Streaming)?**: To achieve minimal latency. Each event is processed as soon as it arrives to detect anomalies or monitor live activity on the Streamlit dashboard. Ideal for immediate responsiveness.

### 2. Batch Layer (Historical)
- **Flow**: MongoDB → Airflow → BigQuery → dbt.
- **Role**: Provide a comprehensive and ultra-precise view of all historical data. This is where dbt transforms raw data into reliable KPIs for the QlikSense dashboard.
...
This platform processes massive event streams (user activity and Wikimedia changes) to provide real-time performance indicators (KPIs) and historical analysis.

### 🔧 Components

| Layer | Technology | Role |
|-------|------------|---------|
| **Ingestion** | Kafka, Schema Registry | Collect Avro streams (User Activity & Wikimedia) |
...
| **Orchestration** | Airflow | Scheduling DAGs, batch jobs, and dbt |
| **Transformation** | dbt Core | SQL modeling, KPIs & Data Quality |
| **Visualization** | Streamlit / Qlik | Real-time dashboards and BI steering |

---

## 📊 Analytical Results & KPIs

The pipeline produces analytics-ready tables in BigQuery:
...
| **Airflow** | [http://localhost:8082](http://localhost:8082) | Orchestration & Pipeline Monitoring |
| **Kafka UI** | [http://localhost:9021](http://localhost:9021) | Topic Management & Avro Schemas |
| **Spark UI** | [http://localhost:9090](http://localhost:9090) | Streaming Job Monitoring |

---

## 🔐 Data Quality (DataOps via dbt)

Data reliability is ensured by automated dbt tests:
